...
const Bfake = fs.readFileSync ('./media/image/fake.jpeg','base64')
		...
		let senderr = mek.key.fromMe ? farz.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = farz.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","listType": "SINGLE_SELECT","sections": list}}, {quoted : mek})
            return farz.relayWAMessage(po, {waitForAck: true})
        }
        ...
        const reply = (teks) => {
	      farz.sendMessage(from, teks, text ,{quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})	 
        }
        const sendMess = (hehe, teks) => {
           farz.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? farz.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/Farz2.jpg')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : farz.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/Farz2.jpg')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           farz.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return farz.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/Farz2.jpg')})
        }
        const freply = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { "contactMessage": { "displayName": `${pushname}`, "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${senderr.split('@')[0]}:${senderr.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, "jpegThumbnail":fs.readFileSync('./media/Farz1.jpg')
        }}}
       const math = (teks) => {
      ...
      const hideTag = async function(from, text){
	       ...
	       farz.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('./media/Farz1.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
      const sendWebp = async(to, url) => {
           ...
           farz.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:mek})
           fs.unlinkSync(asw)
});
});
});
      ...
      const promoteAdmin = async function(to, target=[]){
           ...
           let g = await farz.groupMetadata(to)
           let owner = g.owner.replace("c.us","s.whatsapp.net")
           let me = farz.user.jid
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await farz.groupMakeAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
      ...
      const demoteAdmin = async function(to, target=[]){
           ...
           for (i of target){
           if (!i.includes(me) && !i.includes(owner)){
           const res = await farz.groupDemoteAdmin(to, [i])
           }else{
           reply("NOT PREMITED")
}
}
      ...
      // Anti link
        if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (budy.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                farz.groupRemove(from, [sender])
            }
        }
        
        // Sewa
             _sewa.expiredCheck(farz, sewa)
        ...
        // MUTE
			...
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker...'))
				...
				download(url, namea, async function () {
					...
					farz.sendMessage(to, media, sticker)
...
✓ *Sejak* : ${getTime}`
      mentions(cptl, x, true)
	    ...
	    // Auto Read
        //farz.chatRead(from, "read")
        
       // CMD
        if (isCmd && !isGroup)
		    atm.addKoinUser(sender, randomNomor(20), _uang)
            console.log(color('[ CMD ]','blue'), color(time, 'deeppink'), color(`${command} [${args.length}]`,'orange'), color('from','deeppink'), color(`${pushname}`,'orange'))
        
        if (isCmd && isGroup)
            atm.addKoinUser(sender, randomNomor(20), _uang)
            console.log(color('[ CMD ]','blue'), color(time, 'deeppink'), color(`${command} [${args.length}]`,'orange'), color('from','deeppink'), color(`${pushname}`,'orange'), 'in', color(`${groupName}`,'blue'))
		...
		if (isRegister) return 
		    ...
		    farz.sendMessage(from, teks, text, {quoted: freply })
}
	              switch(command){
        ...
        case 'creator':
               ...
               await sleep(1000)
               farz.sendMessage(from, `Nih Kak Contact Owner Ku🐣 `, text, {quoted: freply})
               break
...
case 'tes':
        ...
        menu =`*Hi* ,@${sender.split('@')[0]}
        
${shp1} *Owner : ${ownerName}*
${shp1} *Hit Today : ${hit_today.length} Hit*
${shp1} *Status : Online*
${shp1} *Lib : Baileys*
${shp1} *Type : NodeJs*
${shp1} *Prefix : 「 Multi Prefix 」*
${shp1} *Mode : ${public ? "Public-Mode" : "Self-Mode"}*
${shp1} *Platform : ${os.platform()}*
${shp1} *Time : ${time2}*
${shp1} *Speed : ${latensie.toFixed(4)} Second*
${shp1} *Active : ${runtime(process.uptime())}*

  *「 CHAT INFO」*
${shp1} *Group Chats : ${groups.length}*
${shp1} *Private Chats : ${privat.length}*
${shp1} *Total Chats : ${totalChat.length}*
...
${shp}*${prefix}clearall*
${shp}*${prefix}leaveall*
${shp}*${prefix}join* <link gc>
${shp}*${prefix}shutdown*
${shp}*${prefix}getquoted*
${shp}*${prefix}cekapikey*
${shp}*${prefix}addupdate* <fiturnya>
${shp}*${prefix}exif* <nama|author>
${shp}*${prefix}listprem*
${shp}*${prefix}listgroup*
${shp}*${prefix}sewa add/del* 
${shp}*${prefix}premium add* 
${shp}*${prefix}premium del*
...
${shp}*${prefix}gawrgura*
${shp}*${prefix}ttg* <teks>
${shp}*${prefix}attp* <teks>
${shp}*${prefix}stickeranime*
${shp}*${prefix}sticker* <reply foto/video>
${shp}*${prefix}swm* <pack|author>
${shp}*${prefix}delwm* <pack|author>
${shp}*${prefix}take* <pack|author>
${shp}*${prefix}togif* <reply video>
${shp}*${prefix}tomp3* <reply video>
${shp}*${prefix}tovideo* <reply sgif>
...
${shp}*${prefix}tiktokaudio*
${shp}*${prefix}mediafire*

          *「 FUN MENU 」*
          
${shp}*${prefix}slot*
${shp}*${prefix}suit* <batu/kertas/gunting>
${shp}*${prefix}gelud* @tag
${shp}*${prefix}tictactoe* @tag 
${shp}*${prefix}mining*
${shp}*${prefix}togel*
${shp}*${prefix}fitnah*
${shp}*${prefix}cekbapak*
${shp}*${prefix}cekwatak*
${shp}*${prefix}cekmati* <nama>
${shp}*${prefix}wangy* <nama>
${shp}*${prefix}citacita*
${shp}*${prefix}toxic*
${shp}*${prefix}truth*
${shp}*${prefix}dare*
...
${shp}*${prefix}beban*
${shp}*${prefix}babi*
${shp}*${prefix}cekganteng*
${shp}*${prefix}cekcantik*

         *「 GROUPMENU 」*

${shp}*${prefix}groupsetting*
${shp}*${prefix}afk* <alasan>
${shp}*${prefix}ceksewa*
${shp}*${prefix}kickall*
${shp}*${prefix}online*
${shp}*${prefix}infogrup*
${shp}*${prefix}promote*
${shp}*${prefix}demote*
${shp}*${prefix}tagall* <teks>
${shp}*${prefix}leave*
${shp}*${prefix}kick* <reply>
${shp}*${prefix}add* +62xxxxxx
${shp}*${prefix}setgrupname*
${shp}*${prefix}setppgrup*
${shp}*${prefix}setdesc*
${shp}*${prefix}caripesan* <pesannya>
${shp}*${prefix}delete* <reply chat bot>
${shp}*${prefix}sider* <reply chat bot>
${shp}*${prefix}hidetag* <teks/reply teks>

          *「 SESSION & CMD 」*

${shp}*${prefix}jadibot*
${shp}*${prefix}stopjadibot*
${shp}*${prefix}listjadibot*
${shp}*${prefix}setcmd* <!command>
${shp}*${prefix}delcmd* <reply stick>
${shp}*${prefix}listcmd*

          *「 OTHER 」*

${shp}*${prefix}brainly* <query>
${shp}*${prefix}shopee* <product>
${shp}*${prefix}playstore* <query>
${shp}*${prefix}google* <query>
${shp}*${prefix}image* <query>
${shp}*${prefix}pinterest* <query>
${shp}*${prefix}igstalk* <username>
${shp}*${prefix}img2url* <reply foto>
${shp}*${prefix}ytsearch* <query>
...
${shp}*${prefix}runtime*
${shp}*${prefix}donasi*
${shp}*${prefix}leaderboard*
${shp}*${prefix}cekpremium*
${shp}*${prefix}listpremium*
${shp}*${prefix}sourcecode*
${shp}*${prefix}bugreport* <problem>
  
°さ⃟🕊️ *Note : No Spam Bot* :)
`
...
[ ${args[0]} ] Ketik Y/N untuk menerima atau menolak permainan`
               ...
               farz.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
...
//------------------< Premium >-------------------
       ...
       case 'buypremium':
       case 'sewabot':
              gopeynya = 'https://telegra.ph/file/b57b7eb75afe38d0031dd.jpg'
              teksnya = `*── 「 PRICE LIST 」 ──*
...
*Pembelian Premium yang disertai SewaBot hanya akan membayar 20K (Diskon 5K)*`
              farz.sendMessage(from, await getBuffer(gopeynya), image, {quoted: mek, caption: teksnya })
              break
//------------------< Sticker Cmd >-------------------
       case 'addcmd': 
       case 'setcmd':
...
case 'googlesearch':
case 'ggs':
if (args.length < 1) return farz.sendMessage(from,('Yang mau di cari apaan?'),text ,{quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
teks = args.join(' ')
farz.sendMessage(from,(mess.wait),text ,{quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
...
*[Wait] Tunggu Sebentar..*`
				...
				farz.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: '[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜...'})
					break  
					
					case 'ytdl':
					case 'play':
			  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////if (!isRegister) return reply(`You are not verified\n\nReply this chat and send bot password\n\nHint : \nPassword contains 4 digit number\nCheck password at: https://farz-chan02.github.io`)
              if (!q) return reply('Linknya?')
			 res = await yts(q)
			   let thumbInfo = ` 
*Youtube Download*
...
imageMessage = (await farz.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage
...
• Link : ${get_data[i].url}`
              ...
              farz.sendMessage(from, ini_buffer, image, { quoted: mek, caption: teks })
              } catch {
...
• Link : ${i.url}`
       ...
       case 'img2url':
               ...
               var media = await  farz.downloadAndSaveMediaMessage(encmedia)       
               imgbb('39d895963468b814fad0514bd28787e2', media)
              .then(data => {
               var caps = `*_IMAGE TO URL_*\n\n*~>  ID :* ${data.id}\n*~>  MimeType :* ${data.image.mime}\n*~>  Extension :* ${data.image.extension}\n*~>  URL :* ${data.display_url}`
               ibb = fs.readFileSync(media)
               farz.sendMessage(from, ibb, image, { quoted: mek, caption: caps})
})
              .catch(err => {
               throw err
})
...
//------------------< Level >-------------------
      case 'level': 
              ...
              try {
              profilePic = await farz.getProfilePicture(sender)
              } catch {
              profilePic = errorImg
}
              ...
              farz.sendMessage(from, buffer, image, { caption: teks, quoted: mek})
              break
...
//------------------< Stalk >-------------------
      ...
      case 'stalkig':
      case 'igstalk':
             if (args.length == 0) return reply(`Example: ${prefix + command} farz.chan26`)
             reply(mess.wait)
             username = args[0]
...
`
             farz.sendMessage(from, ini_buffer, image, { caption: ini_txt, thumbnail: Buffer.alloc(0) })
             break
...
//------------------< Sticker/Tools >-------------------
      ...
      case 'doge':
       ...
       case 'emoji':
              ...
              await farz.sendMessage(from, ini_buffer, sticker, { quoted: mek })
              break
case 'ttp':
...
break
       ...
       case 'sticker':
          ...
          case 'sgif':            
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await farz.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											farz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
											fs.unlinkSync(media)	
       ...
       case 'colong':
              ...
              require('./lib/fetch.js').modStick(media, farz, mek, from)
              break
       case 'delwm':
              if (!isQuotedSticker) return reply('Stiker aja om')
       ...
       case 'swm':
             ...
             .on('error', (e) => {
              console.log(e)
              farz.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
              fs.unlinkSync(media)
})
             .on('end', () => {
            _out = getRandom('.webp')
             ...
             .on('exit', () => {
              farz.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
              fs.unlinkSync(out)
              fs.unlinkSync(_out)
              fs.unlinkSync(media)
})
             ...
             .save(out) 
              ...
              const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              const media = await farz.downloadAndSaveMediaMessage(encmedia)
              pe = args.join('')
              var a = pe.split("|")[0];
              var b = pe.split("|")[1];
              await createExif(a,b)
      ...
      case 'toimg':
              ...
              farz.sendMessage(from, buffer, image, {quoted: freply, caption: '[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜...'})
              fs.unlinkSync(ran)
})
              break
        ...
        case 'togif':
               ...
               farz.sendMessage(from, mp4, video, {mimetype: 'video/gif', quoted: mek, caption: mess.success})
...
//------------------< Fun >-------------------
       ...
       case 'toxic':
        ...
        case 'citacita':
              ...
              farz.sendMessage(from, cita2, audio,{mimetype: 'audio/mp4', ptt:true, quoted: mek})
              break
       case 'apakah':
              apakah = body.slice(1)
              const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi']
              const kah = apa[Math.floor(Math.random() * apa.length)]
              farz.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              farz.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
       case 'gantengcek':
       case 'cekganteng':
              ...
              farz.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              farz.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await farz.getProfilePicture(sender)
       ...
       case 'hobby':
              ...
              farz.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
              break
       case 'bisakah':
              bisakah = body.slice(1)
              const bisa =['Bisa','Tidak Bisa','Coba Ulangi','MANA GW TAU']
              const keh = bisa[Math.floor(Math.random() * bisa.length)]
              farz.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
              kapankah = body.slice(1)
              const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
              const koh = kapan[Math.floor(Math.random() * kapan.length)]
              farz.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
              ...
              farz.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
              ...
              farz.sendMessage(from, buffer, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
              break		
       case 'jadian':
              jds = []
              jdii = groupMembers
...
//------------------< Lainnya >-------------------
        case 'getpp':
               ...
               farz.sendMessage(from, buffer, image, {caption: "[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜...", quoted: mek })
        ...
        case 'delete': // MR.CYSER
               ...
               farz.deleteMessage(from, {id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true})
               } catch (e){
               reply('Reply chat bot')
}
               break
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              farz.sendMessage('6285692949920@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
       if (!isOwner) return  reply(mess.only.owner)
              totalchat.map( async ({ jid }) => {
              await farz.chatRead(jid)
})
...
//------------------< Enable/Disable >-------------------
       ...
       if (!isGroupAdmins) return reply(mess.only.admin)
              if (!isGroup) return reply(mess.only.group)
              if (ar[0] === 'enable') {
              if (isLevelingOn) return reply(' 「 LEVELING TELAH DI AKTIFKAN  SEBELUMNYA 」')
            _leveling.push(from)
              fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
              reply(' ```[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜... mengaktifkan leveling di group``` ')
              } else if (ar[0] === 'disable') {
              var anup = _leveling.indexOf(from)
            _leveling.splice(anup, 1)
              fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
              reply(' ```[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜... menonaktifkan leveling di group```')
              } else {
       ...
       if (!isGroup) return reply(mess.only.group)
        ...
        if (!isGroupAdmins) return reply(mess.only.admin)
               ...
               if ((args[0]) === 'enable') {
               if (isWelkom) return reply(' 「 WELCOME TELAH DI AKTIFKAN  SEBELUMNYA 」')
               welkom.push(from)
               fs.writeFileSync('./database/group/welcome.json', JSON.stringify(welkom))
               reply(' ```[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜... mengaktifkan welcome di group```')
               } else if ((args[0]) === 'disable') {
               welkom.splice(from, 1)
               fs.writeFileSync('./database/group/welcome.json', JSON.stringify(welkom))
               reply(' ```[ √ ] 𝚂𝚞𝚌𝚌𝚎𝚜𝚜... menonaktifkan welcome di group```')
               } else {
        ...
        if (!isGroupAdmins) return reply(mess.only.admin)
               ...
               if (args[0].toLowerCase() === 'enable'){
               if (isMuted) return reply(` 「 GROUP TELAH DI MUTE 」 `)
               mute.push(from)
      ...
      case 'grup':
             ...
             if (args[0].toLowerCase() === 'enable'){
             farz.groupSettingChange(from, "announcement", false)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
             } else if (args[0].toLowerCase() === 'disable'){
             farz.groupSettingChange(from, "announcement", true)
            .then((res) => reply(jsonformat(res)))
            .catch((err) => reply(jsonformat(err)))
...
//------------------< OTHER >-------------------
      case 'infoig':
             reply(` *•「 FOLLOW IG OWNER 」•* : https://www.instagram.com/faaaarr__/`)
             break
      case 'farzgroup':
...
• Pemenangnya adalah [ @${winR} ] `
	   farz.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [winR + "@s.whatsapp.net", gelutSkuy.Z + "@s.whatsapp.net", gelutSkuy.Y + "@s.whatsapp.net",]}})
		fs.unlinkSync("./media/" + from + ".json");
	} else if (sender == `${gelutSkuy.Y}@s.whatsapp.net` &&  budy.toLowerCase() == 'n') {
		farz.sendMessage(from, `👑 Game Gelud Rejected 🤙🏻
...
Giliran = @${tty.player1.split('@')[0]}
  ...
  farz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
farz.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
farz.sendMessage(from, ucapan1, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
farz.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
farz.sendMessage(from, {text:teks, jpegThumbnail:fs.readFileSync('./media/Farz2.jpg')}, 'extendedTextMessage', {sendEphemeral:true, quoted:mek, contextInfo:{ forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
}
	} 
